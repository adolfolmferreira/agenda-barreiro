...
import { fmtRange, cleanLoc, mk, mkLabel, MO_FULL } from '../components/helpers';
              ...
              <div className="tsl-dropdown-menu">
                {categories.map(c => (
                  <button key={c} className={`tsl-dropdown-item ${selCat === c ? 'active' : ''}`}
                    onClick={() => { setSelCat(c); setCatOpen(false); }}>{c}</button>
                ))}
              </div>
            )}
          </div>
              ...
              <div className="tsl-dropdown-menu">
                {months.map(m => (
                  <button key={m} className={`tsl-dropdown-item ${selMon === m ? 'active' : ''}`}
                    onClick={() => { setSelMon(m); setMonOpen(false); }}>{m === 'Todos os Meses' ? m : mkLabel(m)}</button>
                ))}
              </div>
            )}
          </div>
          ...
          <section key={mkey} className="tsl-month">
            ...
            <div className={viewMode === 'grid' ? "tsl-grid" : "tsl-list"}>
              {evts.map(ev => viewMode === 'grid' ? (
                ...
                <Link key={ev.id} href={`/evento/${ev.id}`} className="tsl-list-item">
